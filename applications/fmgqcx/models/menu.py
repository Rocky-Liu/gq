...
if session.authorized:
    if session.auth.user.username == 'admin':
        ...
        response.menu.append(('公告栏', _c == 'notice', URL(_a, 'notice', 'index', args=_t)))
        response.menu.append(('留言板', _c == 'message', URL(_a, 'message', 'index', args=_t)))
        response.menu.append(('用户管理', _c == 'user', URL(_a, 'user', 'index', args=_t)))
    else:
        response.menu = [('首页', _c == 'default', URL(_a, 'default', 'my', args=_t))]
        response.menu.append(('公告栏', _c == 'notice', URL(_a, 'notice', 'index', args=_t)))
        response.menu.append(('留言板', _c == 'message', URL(_a, 'message', 'index', args=_t)))
        response.menu.append(('修改密码', _c == 'user', URL(_a, 'user', 'me', args=_t)))
